...
describe('Auth Service', () => {
  ...
  describe('signup', () => {
    ...
    it('should successfully create a new user', async () => {
      const result = await authService.signup(email, password, confirmPassword, tosAccepted);

      expect(signupSpy).toHaveBeenCalledWith(email, password, confirmPassword, tosAccepted);
      expect(result).toBe(true);

      // Verify user was actually created
      const users = await pb.collection('teachers').getList(1, 1, { filter: `email = "${email}"` });
      expect(users.items).toHaveLength(1);
      expect(users.items[0].email).toBe(email);
    });
  });
